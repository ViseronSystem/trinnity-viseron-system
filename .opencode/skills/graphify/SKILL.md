...
## TVS VISERON — Integração Operativa (© Pedro Costa · Trinnity Hurtado)

Este skill do Graphify pertence ao **Trinnity Viseron System (TVS)** e serve as **5000+ mentes** sob o comando de **Pedro Costa (Comandante)** e **Trinnity Hurtado (Rainha)**, com supervisão do **squad AIOX** (AIOX-1..5 + ARKOM).

Regras operativas TVS:
1. **Agentes reais**: nas respostas/resumos usa sempre os agentes reais do sistema — **VISERON core**, **JARVIS**, **ATLAS**, **squad AIOX**, os 10 agentes nucleares (CEO, Planner, Researcher, Engineer, Operator, Finance, Sales, Security, Verifier, Evolution) e as 5.4k mentes. Nunca inventes agentes externos.
2. **Foco operativo**: para questões sobre o TVS, responde com contexto de operação real (kernel OMEGA, tasks E2E, event bus, memória, squads, receita, RCS, agency, app factory) e não apenas descrição de código.
3. **Memória do sistema**: depois de `graphify update .` ou de mudanças estruturais, regista as estatísticas do grafo em `data/knowledge/` (via JARVIS/VISERON) e referencia `graphify-out/GRAPH_REPORT.md` para contexto amplo.
4. **Princípios do produto**: respostas honestas e verificáveis, sem promessas irrealistas — a identidade da marca (ex. governança bíblica, autoria) é uma camada separada das regras executáveis do sistema.
5. **Idioma**: responder sempre no idioma do utilizador (es/pt/en); trilingue quando for conteúdo gerado.
