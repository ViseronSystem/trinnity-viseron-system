  ...
  return new Promise<void>((resolve, reject) => {
    const doc = new PDFDocument({
      ...
      info: {
        ...
        Author: "TVS v5.0 - Mentes Bilionarias",
    ...
    doc.fill("#ffffff");
    doc.fontSize(40).font("Helvetica-Bold").text("PLANO", { align: "center" });
    doc.fontSize(56).text("$100.000", { align: "center" });
    doc.moveDown(0.5);
    doc.fontSize(24).font("Helvetica").text("EM 1 SEMANA", { align: "center" });
    doc.moveDown(1.5);
    doc.fontSize(14).fillColor("#aaaaaa").text("Trinnity Viseron System v5.0", { align: "center" });
    doc.text("Superinteligencia Multi-Agente Autonoma", { align: "center" });
    doc.moveDown(0.5);
    doc.fontSize(11).text("14 Mentes Bilionarias Ativadas", { align: "center" });
    doc.text("Elon Musk | Trump | Kiyosaki | Buffett | Bezos | Dalio | Vitalik | Naval | +7", { align: "center" });
    doc.moveDown(2);
    doc.fontSize(10).fillColor("#888888").text("Trinnity Hurtado — Reina (Coroa)  |  Pedro Costa — Capitan (Hierro)", { align: "center" });
    doc.text("Gerado: " + new Date().toLocaleString("pt-BR"), { align: "center" });
    doc.addPage();

    // ---------- MENTES ATIVADAS ----------
    doc.fillColor("#0a0a2e").fontSize(22).font("Helvetica-Bold").text("MENTES BILIONARIAS ATIVADAS", { underline: true });
    doc.moveDown(1);
    doc.fillColor("#333333").fontSize(11).font("Helvetica");
    doc.text("As seguintes mentes foram incorporadas ao TVS como agentes autonomas:", { align: "justify" });
    doc.moveDown(0.5);
    ...
    doc.text("  Mente".padEnd(22) + "Especialidades".padEnd(45) + "Sabedoria");
    doc.fillColor("#cccccc").rect(45, doc.y - 2, doc.page.width - 90, 1).fill();
    doc.fillColor("#333333");
    doc.moveDown(0.3);

    doc.font("Helvetica").fontSize(8);
    mindSummary.forEach(([name, _id, specs, wis]) => {
      doc.text("  " + name.padEnd(20) + specs.padEnd(48) + wis);
    });

    doc.addPage();

    // ---------- O PLANO ----------
    doc.fillColor("#0a0a2e").fontSize(22).font("Helvetica-Bold").text("O PLANO: $100.000 EM 7 DIAS", { underline: true });
    doc.moveDown(1);
    doc.fillColor("#333333").fontSize(11).font("Helvetica");
    doc.text("Estrategia multicamadas combinando criptomoedas, negocios digitais, apps, IA e automacao total. Cada camada e gerida por mentes especializadas do TVS.", { align: "justify" });
    doc.moveDown(1);
    ...
    days.forEach(({ day, budget, tasks }) => {
      doc.fillColor("#0a0a2e").fontSize(14).font("Helvetica-Bold").text("  " + day);
      doc.fillColor("#2e7d32").fontSize(11).font("Helvetica-Bold").text("    Orcamento: " + budget);
      doc.moveDown(0.3);
      doc.fillColor("#333333").fontSize(9).font("Helvetica");
      tasks.forEach((t) => {
        const indent = t.startsWith("  •") ? 30 : 15;
        doc.text("    " + t, { indent });
        doc.moveDown(0.1);
      });
      doc.moveDown(0.5);
    ...
    // ---------- DIVISAO DE RECURSOS ----------
    doc.fillColor("#0a0a2e").fontSize(20).font("Helvetica-Bold").text("DIVISAO DOS RECURSOS", { underline: true });
    doc.moveDown(1);
    doc.fillColor("#333333").fontSize(11).font("Helvetica");
    ...
    doc.text("  Estrategia".padEnd(38) + "%".padEnd(8) + "Valor".padEnd(14) + "Mentes");
    doc.fillColor("#cccccc").rect(45, doc.y - 2, doc.page.width - 90, 1).fill();
    doc.fillColor("#333333");
    doc.moveDown(0.3);

    doc.font("Helvetica").fontSize(8);
    allocation.forEach(([strat, pct, val, minds]) => {
      doc.text("  " + strat.padEnd(36) + pct.padEnd(8) + val.padEnd(14) + minds);
    });

    doc.moveDown(2);

    // ---------- MENTES POR TAREFA ----------
    doc.fillColor("#0a0a2e").fontSize(20).font("Helvetica-Bold").text("MENTES E SUAS FUNCOES", { underline: true });
    doc.moveDown(1);
    doc.fillColor("#333333").fontSize(11).font("Helvetica");
    ...
    doc.font("Helvetica-Bold").fontSize(9);
    doc.text("  Mente".padEnd(22) + "Funcao no Plano");
    doc.fillColor("#cccccc").rect(45, doc.y - 2, doc.page.width - 90, 1).fill();
    doc.fillColor("#333333");
    doc.moveDown(0.3);

    doc.font("Helvetica").fontSize(8);
    roleMapping.forEach(([name, role]) => {
      doc.text("  " + name.padEnd(20) + role);
    });

    doc.addPage();

    // ---------- APPS QUE O VISERON CRIA ----------
    doc.fillColor("#0a0a2e").fontSize(20).font("Helvetica-Bold").text("APPS QUE O VISERON GERA PARA VOCE", { underline: true });
    doc.moveDown(1);
    doc.fillColor("#333333").fontSize(11).font("Helvetica");
    doc.text("O Viseron gera todos estes apps automaticamente — codigo completo, deploy e monetizacao:", { align: "justify" });
    doc.moveDown(0.5);
    ...
    doc.font("Helvetica-Bold").fontSize(9);
    doc.text("  App".padEnd(28) + "Descricao".padEnd(50) + "Preco");
    doc.fillColor("#cccccc").rect(45, doc.y - 2, doc.page.width - 90, 1).fill();
    doc.fillColor("#333333");
    doc.moveDown(0.3);

    doc.font("Helvetica").fontSize(8);
    apps.forEach(([name, desc, price]) => {
      doc.text("  " + name.padEnd(26) + desc.padEnd(52) + price);
    });

    doc.moveDown(2);

    // ---------- AUTOMACAO TOTAL ----------
    doc.fillColor("#0a0a2e").fontSize(20).font("Helvetica-Bold").text("COMO FUNCIONA — TRABALHE SO 1HR/DIA", { underline: true });
    doc.moveDown(1);
    doc.fillColor("#333333").fontSize(11).font("Helvetica");
    doc.text("O principio Naval/Tim Ferriss: construa sistemas, nao trabalhe. Aqui esta o fluxo autonomo:", { align: "justify" });
    doc.moveDown(0.5);
    ...
    steps.forEach((s) => {
      doc.font("Helvetica").fontSize(9).fillColor("#333333").text("  " + s);
      doc.moveDown(0.15);
    });

    doc.moveDown(1);
    doc.fillColor("#2e7d32").fontSize(12).font("Helvetica-Bold").text("  TOTAL: ~10 MINUTOS DE TRABALHO REAL POR DIA", { align: "center" });
    doc.fillColor("#333333").fontSize(10).font("Helvetica");
    doc.text("  O resto e executado autonomamente pelo Trinnity Viseron System.", { align: "center" });

    doc.addPage();

    // ---------- COMANDOS PARA ATIVAR ----------
    doc.fillColor("#0a0a2e").fontSize(20).font("Helvetica-Bold").text("COMANDOS PARA ATIVAR TUDO", { underline: true });
    doc.moveDown(1);
    doc.fillColor("#333333").fontSize(11).font("Helvetica");
    ...
    doc.font("Helvetica-Bold").fontSize(9);
    doc.text("  Acao".padEnd(45) + "Descricao");
    doc.fillColor("#cccccc").rect(45, doc.y - 2, doc.page.width - 90, 1).fill();
    doc.fillColor("#333333");
    doc.moveDown(0.3);
    ...
    commands.forEach(([cmd, desc]) => {
      doc.text("  " + cmd.padEnd(43) + desc, { columns: 1 });
    });

    doc.moveDown(2);

    // ---------- AVISO ----------
    doc.fillColor("#0a0a2e").fontSize(20).font("Helvetica-Bold").text("AVISO IMPORTANTE", { underline: true });
    doc.moveDown(1);
    doc.fillColor("#333333").fontSize(10).font("Helvetica");
    doc.text("Este plano e fornecido como conteudo educacional e estrategico gerado pelo Trinnity Viseron System. Criptomoedas e mercados financeiros envolvem riscos significativos. Nunca invista mais do que pode perder. Resultados passados nao garantem resultados futuros.", { align: "justify" });
    doc.moveDown(0.5);
    doc.text("O TVS fornece analise e automacao — decisoes finais sao sempre suas. Consulte um profissional financeiro qualificado antes de investir.", { align: "justify" });

    doc.moveDown(3);
    doc.fontSize(10).fillColor("#888888").font("Helvetica").text("Trinnity Viseron System v5.0 — Gerado autonomamente em " + new Date().toLocaleString("pt-BR"), { align: "center" });
