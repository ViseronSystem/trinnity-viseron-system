...
if (-not $registered) {
    ...
    Write-Host "       - Tipo A, nome @,  IP 76.76.21.21 (Vercel), Proxy OFF (DNS only, nuvem cinzenta)"
    Write-Host "       - Tipo CNAME, nome www, alvo cname.vercel-dns.com, Proxy OFF (DNS only)"
    Write-Host "       NOTA: com proxy ON (nuvem laranja) o site fica offline com HTTP 522"
    Write-Host "       (Cloudflare nao alcanca a origem Vercel). Vercel exige DNS-only."
